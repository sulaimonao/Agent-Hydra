...
export const orchestrateContextWorkflow = async ({
  ...
  try {
    ...
    console.log('appendMemory params:', { query, user_id, chatroom_id });
    const updatedMemory = appendMemory(query, user_id, chatroom_id);
    updatedContext.memory = updatedMemory;

    // Create a task card
    const taskCard = createTaskCard(query, actionItems, user_id, chatroom_id);
    ...
    const context = updateContext(updatedContext, user_id, chatroom_id);
    ...
    console.log('taskCard before final response:', taskCard);
